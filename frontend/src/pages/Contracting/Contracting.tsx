...
const Contracting: React.FC = () => {
  ...
  const [selectedContractors, setSelectedContractors] = useState<Set<number>>(
    new Set()
  );
  ...
  const [linkCopySuccess, setLinkCopySuccess] = useState<number | null>(null);
  const [visibleContractorsCount, setVisibleContractorsCount] = useState(3);
  const [contractorDescriptions, setContractorDescriptions] = useState<
    Map<number, string>
  >(new Map());
  ...
  const generateDefaultEmailBody = useCallback((): string => {
    ...
    const projectTypeLabel =
      PROJECT_TYPES.find((pt) => pt.value === selectedProject.project_type)
        ?.label || selectedProject.project_type;
    const address =
      [selectedProject.city, selectedProject.state]
        .filter(Boolean)
        .join(", ") || selectedProject.address;
...
- Budget: ${budget}
${
  selectedProject.additional_information
    ? `- Additional Information: ${selectedProject.additional_information}`
    : ""
}
...
Best regards`;
  ...
  const handleSendEmail = async () => {
    ...
    const contractorsWithoutEmail = selectedContractorsList.filter(
      (c) => !c.email
    );
    if (contractorsWithoutEmail.length > 0) {
      setError(
        `Some contractors don't have email addresses: ${contractorsWithoutEmail
          .map((c) => c.name)
          .join(", ")}`
      );
      return;
  ...
  const handleGenerateLinks = async () => {
    ...
    setError(null);
    try {
      const resp: any = await contractorApi.generateUploadLinks(
        selectedProject.id!,
        Array.from(selectedContractors)
      );
      const linksArray = resp?.links ?? resp;
      const map = new Map<number, string>();
      (linksArray || []).forEach((item: any) => {
        map.set(item.contractorId, item.uploadUrl);
      });
      setGeneratedLinks(map);
    } catch (err) {
      setError(
        err instanceof Error ? err.message : "Failed to generate upload links"
      );
    } finally {
  ...
  const generateWhyThisContractor = useCallback(
    async (contractor: Contractor): Promise<string> => {
      if (!selectedProject) return "";
      ...
      return new Promise((resolve) => {
        setTimeout(() => {
          ...
          const projectTypeLabel =
            PROJECT_TYPES.find(
              (pt) => pt.value === selectedProject.project_type
            )?.label || selectedProject.project_type;
          ...
          if (rating && rating >= 4.5) {
            reasons.push(
              `Excellent rating of ${rating.toFixed(1)}/5.0 with ${
          ...
          if (
            ...
            contractor.years_in_business >= 10
          ) {
            reasons.push(
              `${contractor.years_in_business} years of proven experience in the industry`
            );
          } else if (
            contractor.years_in_business &&
            contractor.years_in_business >= 5
          ) {
            reasons.push(
              `Established business with ${contractor.years_in_business} years of experience`
            );
          }
          ...
          if (contractor.specializations) {
            ...
            if (
              specializations.includes(selectedProject.project_type) ||
              ...
              reasons.push(
                `Offers specialized services: ${contractor.specializations.substring(
                  0,
                  100
                )}${contractor.specializations.length > 100 ? "..." : ""}`
              );
            }
          }
          ...
          if (contractor.kfw_eligible) {
            reasons.push(
              `KfW eligible, which can help with financing options for your project`
            );
          }
          ...
          if (
            ...
            contractor.city.toLowerCase() ===
              selectedProject.city.toLowerCase() &&
            contractor.state.toLowerCase() ===
              selectedProject.state.toLowerCase()
          ) {
            reasons.push(
              `Local contractor based in ${contractor.city}, ensuring quick response times and local expertise`
            );
          } else if (contractor.city && contractor.state) {
            reasons.push(
              `Located in ${contractor.city}, ${contractor.state}, providing regional knowledge`
            );
          }

          if (contractor.certifications) {
            reasons.push(
              `Certified professional with ${contractor.certifications.substring(
                0,
                80
              )}${contractor.certifications.length > 80 ? "..." : ""}`
            );
          }
          ...
          if (contractor.business_size) {
            ...
            if (size.includes("large") || size.includes("established")) {
              reasons.push(
                `Established ${contractor.business_size} business with resources to handle complex projects`
              );
            } else if (size.includes("small") || size.includes("local")) {
              reasons.push(
                `${contractor.business_size} business offering personalized attention and competitive pricing`
              );
            }
          }
  ...
  const handleLoadMore = () => {
    setVisibleContractorsCount((prev) =>
      Math.min(prev + 3, contractors.length)
    );
  };
      ...
      <div className="space-y-4 sm:space-y-6">
          ...
          <div className="bg-gradient-to-r from-emerald-50 to-teal-50 rounded-lg border-2 border-emerald-200 p-3 sm:p-4 md:p-6 shadow-sm">
            <div className="flex items-center gap-3 mb-4">
              ...
              <div className="flex-1">
                <Heading
                  level={3}
                  className="text-lg sm:text-xl text-gray-900 mb-1"
                >
                  Selected Contractors
                </Heading>
                <Text className="text-sm sm:text-base text-gray-700">
                  {selectedContractors.size} contractor
                  {selectedContractors.size !== 1 ? "s" : ""} selected for
                  invitation
                </Text>
              </div>
            </div>
        ...
        {loading && (
          <div className="flex items-center justify-center py-8 sm:py-12">
            ...
            <Text className="ml-2 sm:ml-3 text-gray-600 text-sm sm:text-base">
              Loading contractors...
            </Text>
          </div>
        )}
            ...
            {contractors.length === 0 ? (
              ...
              <>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3 sm:gap-4">
                  {contractors
                    .slice(0, visibleContractorsCount)
                    .map((contractor) => {
                      const isSelected =
                      ...
                      const hasDescription =
                        contractor.id !== undefined &&
                        contractorDescriptions.has(contractor.id);
                      const isGenerating =
                        contractor.id !== undefined &&
                        generatingDescriptions.has(contractor.id);
                      const description =
                        contractor.id !== undefined
                          ? contractorDescriptions.get(contractor.id)
                          : null;
                  ...
                  <div className="flex justify-center mt-6">
                    ...
                    >
                      ...
                      <span>
                        ...
                        {contractors.length - visibleContractorsCount}{" "}
                        remaining)
                      </span>
                    </button>
                  </div>
      ...
      return (
        <div className="bg-yellow-50 border border-yellow-200 rounded-lg p-4 sm:p-6 text-center">
          ...
          <Text className="text-gray-600 text-sm sm:text-base mb-4">
            Please go back to the Matching step and select contractors to
            invite.
          </Text>
          <button
      ...
      <div className="space-y-4 sm:space-y-6">
        ...
        <div className="bg-gradient-to-r from-blue-50 to-indigo-50 rounded-lg border-2 border-blue-200 p-4 sm:p-6">
          <div className="flex items-center gap-3 mb-4">
            ...
            <div className="flex-1">
              ...
              <Text className="text-sm sm:text-base text-gray-700">
                {selectedContractorsList.length} contractor
                {selectedContractorsList.length !== 1 ? "s" : ""} will receive
                this invitation
              </Text>
            </div>
          </div>
        ...
        {emailSent ? (
          <div className="bg-green-50 border-2 border-green-200 rounded-lg p-4 sm:p-6 text-center">
            ...
            <Heading
              level={3}
              className="text-lg sm:text-xl text-gray-900 mb-2"
            >
              Invitations Sent Successfully!
            </Heading>
            <Text className="text-gray-600 text-sm sm:text-base mb-4">
              Your invitations have been sent to{" "}
              {selectedContractorsList.length} contractor
              {selectedContractorsList.length !== 1 ? "s" : ""}.
            </Text>
            <div className="flex flex-col sm:flex-row gap-3 justify-center">
              ...
              <button
                onClick={() => {
                  setEmailSent(false);
                  setEmailSubject(
                    `Project Invitation: ${selectedProject?.name}`
                  );
          ...
          <div className="bg-white rounded-lg border border-gray-200 p-4 sm:p-6">
            ...
            {error && (
              <div className="bg-red-50 border border-red-200 rounded-lg p-3 sm:p-4 mb-4">
                <Text className="text-red-800 text-sm sm:text-base">
                  {error}
                </Text>
              </div>
            )}
            ...
            <div className="space-y-4 sm:space-y-6">
              ...
              {/* Generated Upload Links Section */}
              {generatedLinks.size > 0 && (
                <div className="mt-6 p-4 bg-blue-50 border border-blue-200 rounded-lg">
                  <div className="flex items-center gap-2 mb-4">
                    <Mail className="w-5 h-5 text-blue-600" />
                    <Heading level={4} className="text-base text-blue-900">
                      Generated Upload Links for Contractors
                    </Heading>
                  </div>
                  <Text className="text-sm text-blue-800 mb-4">
                    Copy these links and include them in your email. Each
                    contractor will have their own unique link to upload their
                    offer.
                  </Text>
                  <div className="space-y-2">
                    {selectedContractors &&
                      Array.from(selectedContractors).map((contractorId) => {
                        ...
                        const uploadUrl = generatedLinks.get(contractorId);
                        return (
                          <div
                            key={contractorId}
                            className="flex items-start gap-3 bg-white p-3 rounded border border-blue-100"
                          >
                            <div className="flex-1 min-w-0">
                            ...
                            {uploadUrl && (
                              <button
                                onClick={() => {
                                  ...
                                  setLinkCopySuccess(contractorId);
                                  setTimeout(
                                    () => setLinkCopySuccess(null),
                                    2000
                                  );
                                }}
                                className="flex-shrink-0 px-3 py-2 bg-blue-600 text-white text-xs font-medium rounded hover:bg-blue-700 transition-colors whitespace-nowrap"
                              >
                                {linkCopySuccess === contractorId
                                  ? "Copied!"
                                  : "Copy Link"}
                              </button>
                            )}
              ...
              <div className="flex flex-col sm:flex-row gap-3 pt-4 border-t border-gray-200">
                ...
                <button
                  ...
                  disabled={generatingLinks || selectedContractors.size === 0}
                  className="flex-1 sm:flex-none bg-blue-600 text-white px-6 py-2.5 sm:py-3 rounded-lg font-medium hover:bg-blue-700 transition-colors disabled:bg-gray-400 disabled:cursor-not-allowed flex items-center justify-center gap-2"
                >
                  {generatingLinks ? (
                    <>
                      <Loader2 className="w-4 h-4 sm:w-5 sm:h-5 animate-spin" />
                    ...
                    <>
                      ...
                      <span>Generate Upload Links</span>
                    </>
                  )}
                </button>
                <button
  ...
  if (!selectedProject) {
    return (
      <div className="text-center py-8 sm:py-12 px-4">
        <Heading
          level={1}
          className="mb-3 sm:mb-4 text-xl sm:text-2xl md:text-3xl"
        >
          No Project Selected
        </Heading>
        <Text className="text-gray-600 mb-4 sm:mb-6 text-sm sm:text-base">
          Please select a project from the Home page to access Contracting
          options.
        </Text>
        <button
          onClick={() => navigate("/")}
